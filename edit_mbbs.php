...
<style>
    ...
    <body>
        <div class="rec">
            <div class="table">
                <h1 class="bg">Edit MBBS College choices</h1><br><br>
                <button onclick="window.print()">Print</button>
                <button><a href="edit_colg_2.php?id=<?php echo $s_id;?>">Back</a></button>
                <button class="del"><a onclick="alert('Do you really want to delete all choices of student?')" href="edit_mbbs_2.php?id=<?php echo $s_id;?>">Delete All Choices</a></button>
                <br><br>
                ...
                $sql="INSERT INTO mbbs_priorities (priority,student_id,college_id)
                ...
                ?>
                <br><br>
                <form action="edit_mbbs.php" method="GET">
                ...
                <select name="college" id="college" class="form-control" required> 
                    ...
                    $sql3="SELECT * FROM colleges c where c.id NOT IN (select college_id from mbbs_priorities where student_id='$s_id')";
                    $result3=mysqli_query($conn,$sql3);
                   
                    if(mysqli_num_rows($result3)>0){
                        while($row3=mysqli_fetch_assoc($result3)){
                            echo "<option value='".$row3['id']."'>".$row3['colg_name']."</option>" ;
                        }
                    }
                    ?>
                </select>
                <select name="priority" class="form-control" required>
                    ...
                    <?php
                    $s_id=$_GET['id'];
                    $sql4="SELECT * FROM m_priority where priority_no NOT IN (select priority from mbbs_priorities where student_id='$s_id') ";
                ...
                $res2=mysqli_query($conn,$sql2);
                    ...
                    while($row2=mysqli_fetch_assoc($res2)){
                        echo "<tr><td>".$row2['priority']."</td><td>".$row2['colg_name']."</td><td>".$row2['abbr']."</td></tr>";
                        }
                    echo "</tbody></table>";
